...
** Last update Mon Mar 21 11:32:20 2016 Nyrandone Noboud-Inpeng
*/
...
void	test_strpbrk()
{
  ...
  printf("asm = [%s] -- NULL NULL\n", my_strpbrk(NULL, NULL));
  printf("c   = POINTEUR NULL\n");

  printf("-\n");

  printf("asm = [%s] -- PointeurNulldeFDP NULL\n", my_strpbrk("PointeurNulldeFDP", NULL));
  printf("c   = POINTEUR NULL\n");
}
...
int	main()
{
  printf("\nLes différences pour les fonctions strcmp, strncmp etc. sont normales si le sujet spécifie qu'il faut retourner la différence entre les caractères. Celles du système se contentent juste de retourner 1, -1 ou 0 !\n\n");

  printf("A vous de comparer les outputs un par un. Les outputs de chaque fonction (ceux du système et ceux de votre asm) sont entre CROCHETS. Ce qui suit n'est que le test qui a été effectué.\n\n");

  printf("%sAllez, amusez-vous bien bande de fdps <3%s\n\n", RED, END);
